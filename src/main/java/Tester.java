        ...
        for (Method m : clazz.getDeclaredMethods()){
            if (m.isAnnotationPresent(Test.class)){
                ...
                if ((beforeMethod == null)){
                    ...
                    throw new RuntimeException("Необходимо не более одного метода с аннотацтей @BeforeSuite");
                }
            } if(m.isAnnotationPresent(AfterSuite.class)){
                if(afterMethod == null){
                    afterMethod = m;
                }else {
                    throw new RuntimeException("Необходимо не более одно метода с аннтотацией @AfterSuite");
                }
            }
        }
        if(beforeMethod != null){
